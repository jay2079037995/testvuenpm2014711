...
export default () => {
  ...
  const search = () => {
    ...
    if (textList.length) {
      ...
      message.warning(i18n.global.t('toast.noMatchesFound'))
      clearMarks()
    }
  }
  ...
  const highlightCurrentSlide = () => {
    ...
    setTimeout(() => {
      for (let i = 0; i < searchResults.value.length; i++) {
        ...
        if (slide && undefined !== slide && target.slideId !== slide.id) continue
  ...
  const turnTarget = () => {
    ...
    const slide = currentSlide.value
    if (undefined !== slide && slide && target.slideId === slide.id) setTimeout(setActiveMark, 0)
    else {
  ...
  const searchNext = () => {
    if (!searchWord.value) return message.warning(i18n.global.t('toast.enterSearchContent'))
  ...
  const replace = () => {
    ...
    if (target.elType === 'text') {
      ...
      const el = elements.find(item => item.id === target.elId)
