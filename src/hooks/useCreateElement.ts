...
export default () => {
  ...
  const createElement = (element: PPTElement, callback?: () => void, slideId?:string) => {
    slidesStore.addElement(element, slideId)
  ...
  const createChartElement = (type: PresetChartType) => {
    const newElement: PPTChartElement = {
      ...
      data: {
        labels: [i18n.global.t('element.category1'), i18n.global.t('element.category2'), i18n.global.t('element.category3'), i18n.global.t('element.category4'), i18n.global.t('element.category5')],
    ...
    const chartStr = JSON.stringify(chartElement)
  ...
  const createTableElement = (row: number, col: number) => {
    ...
    const tableStr = JSON.stringify(tableElement)
   ...
   * @param content 文本内容
   */
  const createTextElement = (position: CommonElementPosition, data?: CreateTextData, slideId?:string) => {
  ...
  const createShapeElement = (position: CommonElementPosition, data: ShapePoolItem, supplement: Partial<PPTShapeElement> = {}) => {
    ...
    const shapeStr = JSON.stringify(newElement)
  ...
  const createLineElement = (position: LineElementPosition, data: LinePoolItem) => {
    ...
    if (data.isBroken) {
      const broken: [number, number] = [(start[0] + end[0]) / 2, (start[1] + end[1]) / 2]
      newElement.brokens = [broken]
    } 
