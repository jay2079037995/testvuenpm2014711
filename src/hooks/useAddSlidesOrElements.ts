...
export default () => {
  ...
  const addElementsFromData = (elements: PPTElement[]) => {
    ...
    console.log('addElementsFromData groupIdMap: %o, elIdMap:%o', groupIdMap, elIdMap)
    const firstElement = elements[0]
    let offset = 0
    let lastSameElement: PPTElement | undefined
    const slide = currentSlide.value
    const currentElements = slide && undefined !== slide ? slide.elements : []
    
    do {
      lastSameElement = currentElements.find(el => {
  ...
  const addSlidesFromData = (slides: Slide[]) => {
    ...
    const newSlides = slides.map(slide => {
      ...
      console.log('addSlidesFromData groupIdMap: %o, elIdMap:%o', groupIdMap, elIdMap)
  ...
  const clearAllSlides = () => {
    if (slides.value.length > 1 || slides.value[0].elements.length > 0) {
      slidesStore.clearAllSlide()
    }
    console.log('clearAllSlides isEmptySlide:%d', isEmptySlide)
  }
