...
import pptxgen from '@pagepeek/generate-pptx'
...
export default () => {
  ...
  const { slides, theme, viewportRatio, title, slideSizeWidth } = storeToRefs(slidesStore)
  ...
  const exportImage = (domRef: HTMLElement, format: string, quality: number, ignoreWebfont = true) => {
    ...
    setTimeout(() => {
      ...
      toImage(domRef, config).then(dataUrl => {
        ...
        exporting.value = false
        message.error(i18n.global.t('toast.exportImageFailed'))
      })
    }, 200)
  }
  
  // // 导出GenerativePPT文件（特有 .GenerativePPT 后缀文件）
  // const exportSpecificFile = (_slides: Slide[]) => {
  //   const blob = new Blob([encrypt(JSON.stringify(_slides))], { type: '' })
  //   saveAs(blob, `${title.value}.GenerativePPT`)
  // }
  
  // 导出JSON文件
  const exportJSON = () => {
    const exportData = { slides: slides.value }
    const blob = new Blob([JSON.stringify(exportData)], { type: '' })
    ...
    const parse = (obj: AST[], baseStyleObj: { [key: string]: string } = {}) => {
      for (const item of obj) {
        ...
        if ('attributes' in item) {
          ...
          if (styleAttr && styleAttr.value) {
            ...
            for (const styleItem of styleArr) {
              const [key, value] = styleItem.split(':').map(str => str.trim())
              if (key && value) {
                if ('text-align' === key) {
                  if ('tagName' in item && item.tagName === 'sapn') {
        ...
        if ('tagName' in item) {
          ...
          switch (item.tagName) {
            ...
            case 'sub':
              ...
              break
            case 'a':
              styleObj['href'] = item.attributes.find(attr => attr.key === 'href')?.value || ''
              break
            case 'ul':
        ...
        if ('tagName' in item && item.tagName === 'br') {
          ...
          let fontSize = styleObj['font-size'] ? parseInt(styleObj['font-size']) : undefined

          if (fontSize) {
            fontSize = Math.floor(fontSize * (slideSizeWidth.value / 1000) * PT_PX_RATIO)
          }

          options.fontSize = fontSize
          options.color = styleObj['color'] || undefined
          options.fontFace = styleObj['font-family'] || undefined
          options.bold = styleObj['font-weight'] === 'bold' ? true : undefined
          options.italic = styleObj['font-style'] === 'italic' ? true : undefined
          options.underline = styleObj['text-decoration-line']?.includes('underline') ? { color: options.color || '#000000', style: 'sng' } : undefined
          options.strike = styleObj['text-decoration-line']?.includes('line-through') ? 'sngStrike' : undefined
          options.superscript = styleObj['vertical-align'] === 'super' ? true : undefined
          options.subscript = styleObj['vertical-align'] === 'sub' ? true : undefined
          options.align = styleObj['text-align'] as pptxgen.HAlign || undefined
          options.hyperlink = styleObj['href'] ? { url: styleObj['href'] } : undefined
    ...
    for (const slide of _slides) {
      ...
      for (const el of slide.elements) {
        if (el.type === 'text') {
          ...
          const regex = /^(&nbsp;|\s)+$/
          if (!regex.test(textDiv.innerText) && '' !== textDiv.innerText) {
            const textProps = formatHTML(el.content)

            const options: pptxgen.TextPropsOptions = {
              x: el.left / INCH_PX_RATIO,
              y: (el.top + 5) / INCH_PX_RATIO,
            ...
            // console.log('export text html content: el : o%, parse result : o% , options: o%', el, textProps, options)
          ...
          const chartColorsOpacityArray = el.themeColor.map(color => formatColor(color).alpha * 100)
          ...
          const options: pptxgen.IChartOpts = {
            ...
            chartColorsOpacityArray: chartColorsOpacityArray,
          ...
          if (el.chartType === 'bar') {
            ...
            // 值轴  val axis  api
            // 设置步距离
            // options.valAxisMajorUnit = 0.5
          }
          else if (el.chartType === 'line') {
            if (el.options?.showArea) type = pptx.ChartType.area
            else if (el.options?.showLine === false) {
              type = pptx.ChartType.scatter
              const firstSeriesLength = (el.data.series && el.data.series.length > 0) ? (Array.isArray(el.data.series[0]) ? el.data.series[0].length : el.data.series.length) : 0
              chartData.unshift({ name: 'X-Axis', values: Array(firstSeriesLength).fill(0).map((v, i) => i) })
              options.lineSize = 0
            }
          ...
          let points = formatPoints(toPoints(el.path), scale)

          let lastIndex = points.length - 1

          while (lastIndex >= 0 && ('close' in points[lastIndex]) && !('x' in points[lastIndex])) {
            lastIndex--
          }

          if (lastIndex >= 0 && lastIndex + 2 <= points.length) {
            points = points.slice(0, lastIndex + 2)
          }
          ...
          if (el.fill === 'none') options.fill = undefined
          let shapeName
          if (el.pathFormula) {
            shapeName = el.pathFormula
          } 
          else {
            shapeName = 'custGeom' 
          }

          pptxSlide.addShape(shapeName as pptxgen.ShapeType, options)
          if (el.text) {
            const textDiv: HTMLDivElement | null = document.createElement('div')
            textDiv.innerHTML = el.text.content
            const regex = /^(&nbsp;|\s)+$/
            if (!regex.test(textDiv.innerText) && '' !== textDiv.innerText) {
          ...
          const options: pptxgen.ShapeProps = {
            ...
            line: {
              color: c.color, 
              transparency: (1 - c.alpha) * 100,
              width: Math.max(el.width * PT_PX_RATIO, 1), 
          ...
          for (let i = 0; i < el.data.length; i++) {
            ...
            for (let j = 0; j < row.length; j++) {
              ...
              if (!hiddenCells.includes(`${i}_${j}`)) {
                const text = extractTextFromHTML(cell.text)
                _row.push({
                  text: text,
                  options: cellOptions,
                })
