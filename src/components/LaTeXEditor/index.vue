<template>
  <div class="latex-editor">
    <div class="container">
      <div class="left">
        <div class="input-area">
          <TextArea v-model:value="latex" :placeholder="$t('toast.placeholderLaTeX')" ref="textAreaRef" />
        </div>
        <div class="preview">
          <div class="placeholder" v-if="!latex">{{ $t('latex.formulaPreview') }}</div>
    ...
    <div class="footer">
      ...
      <Button class="btn" type="primary" @click="update()">{{ $t('sure') }}</Button>
    </div>
  </div>
</template>
...
import Tabs from '../TabsSecondary.vue'
...
const tabs: TabItem[] = [
  { label: i18n.global.t('latex.commonSymbol'), key: 'symbol' },
  { label: i18n.global.t('latex.presetFormula'), key: 'formula' },
]
...
const update = () => {
  if (!latex.value) return message.error(i18n.global.t('latex.formulaEmpty'))
...
const placeholderLaTeX = computed(() => {
  return i18n.global.t('toast.placeholderLaTeX')
})
</script>
