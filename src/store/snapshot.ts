...
export const useSnapshotStore = defineStore('snapshot', {
  ...
  actions: {
    ...
    async addSnapshot() {
      ...
      // https://github.com/pipipi-pikachu/GenerativePPT/issues/27
      if (snapshotLength >= 2) {
        db.snapshots.update(allKeys[snapshotLength - 2] as number, { index: slidesStore.slideIndex })
      }

      // const filteredKeys = needDeleteKeys.filter((key): key is number => typeof key === 'number')
      // await db.snapshots.bulkDelete(filteredKeys)
