...
export interface MainState {
  ...
  createDocumentState: boolean
  createNewPPTXState: boolean
}
...
export const useMainStore = defineStore('main', {
  state: (): MainState => ({
    ...
    createDocumentState: false, // 创建文档
    createNewPPTXState: false, // 创建新的pptx
  }),
  ...
  actions: {
    getElementSlideIndex(element: any, pageIndex = -1) {
      console.log('getElementSlideIndex', element.id, pageIndex)
      const slidesStore = useSlidesStore()
      for (let i = 0; i < slidesStore.slides.length; i++) {
        if (pageIndex !== -1 && i !== pageIndex) continue
        for (let j = 0; j < slidesStore.slides[i].elements.length; j++) {
          if (slidesStore.slides[i].elements[j] === element) {
            return i
          }
        }
      }
      for (let i = 0; i < slidesStore.slides.length; i++) {
        if (pageIndex !== -1 && i !== pageIndex) continue
        for (let j = 0; j < slidesStore.slides[i].elements.length; j++) {
          if (slidesStore.slides[i].elements[j].id === element.id) {
            return i
          }
        }
      }
    ...
    setActiveElementIdList(activeElementIdList: string[]) {
      ...
      console.log('setActiveElementIdList:', activeElementIdList)
    },
    ...
    setCreateDocumentState(show: boolean) {
      this.createDocumentState = show
    },

    setCreateNewPPTXState(show: boolean) {
      this.createNewPPTXState = show
    },
  },
})