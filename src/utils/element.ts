...
export const createElementIdMap = (elements: PPTElement[]) => {
  ...
  const elIdMap: IdMap = {}
  console.log('createElementIdMap elements: %o', elements)
...
export const getLineElementPath = (element: PPTLineElement) => {
  ...
  const brokensLength = element.brokens && element.brokens.length > 0 ? element.brokens.length : 0
  if (brokensLength === 1) {
    const broken = element.brokens![0]
    const mid = broken.join(',')
    return `M${start} L${mid} L${end}`
  }
  else if (brokensLength === 2) {
    const b1 = element.brokens![0]
    const b2 = element.brokens![1]
    const l1 = b1.join(',')
    const l2 = b2.join(',')
    return `M${start} L${l1} L${l2} L${end}`
  }
  else if (brokensLength > 2) {
    // let lineD = `M${start} `
    const lineDParts = [`M${start} `]
    for (let i = 0; i < brokensLength; i++) {
      const b = element.brokens![i]
      const l = b.join(',')
      // lineD += `L${l} `
      lineDParts.push(`L${l} `)
    }
    // lineD += `L${end}`
    // return lineD
    lineDParts.push(`L${end}`)
    return lineDParts.join('')
  }
