...
const shadow: MarkSpec = {
  attrs: {
    shadow: {},
  },
  parseDOM: [{
    style: 'text-shadow',
    getAttrs: shadow => shadow ? { shadow } : {}
  }],
  toDOM(mark) {
    const { shadow } = mark.attrs
    let style = ''
    if (backcolor) style += `text-shadow: ${shadow};`
    return ['span', { style }, 0]
  }
}
...
const marL: MarkSpec = {
  ...
  parseDOM: [{
    style: 'margin-left',
    getAttrs: marginLeft => marginLeft ? { marginLeft } : {}
  }],
  toDOM(mark) {
    const { marginLeft } = mark.attrs
    let style = ''
...
const padL: MarkSpec = {
  ...
  parseDOM: [{
    style: 'padding-left',
    getAttrs: paddingLeft => paddingLeft ? { paddingLeft } : {}
  }],
  toDOM(mark) {
    const { paddingLeft } = mark.attrs
    let style = ''
...
const combinedStyleMark: MarkSpec = {
  attrs: {
    background: { default: null },
    backgroundClip: { default: null },
    webkitBackgroundClip: { default: null },
  },
  inline: true,
  group: 'inline',
  parseDOM: [
    {
      ...
      getAttrs: dom => {
        ...
        const backgroundClipMatch = style?.match(/background-clip:\s*([^;]+)/)
        const webkitBackgroundClipMatch = style?.match(/-webkit-background-clip:\s*([^;]+)/)
        return {
          background: backgroundMatch ? backgroundMatch[1] : null,
          backgroundClip: backgroundClipMatch ? backgroundClipMatch[1] : null,
          webkitBackgroundClip: webkitBackgroundClipMatch ? webkitBackgroundClipMatch[1] : null,
        }
      }
    }
  ],
  toDOM: mark => {
    const { background, backgroundClip, webkitBackgroundClip } = mark.attrs
    let style = ''
    if (background) style += `background: ${background};`
    if (backgroundClip) style += `background-clip: ${backgroundClip};`
    if (webkitBackgroundClip) style += `-webkit-background-clip: ${webkitBackgroundClip};`
    return ['span', { style }, 0]
  }
}
