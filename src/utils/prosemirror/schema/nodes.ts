...
const paragraph: NodeSpec = {
  ...
  parseDOM: [
    {
      tag: 'p',
      getAttrs: dom => {
        const { textAlign, textIndent, marginTop} = (dom as HTMLElement).style
        ...
        const top = marginTop || '0px'

        return { align, indent, textIndent: textIndentLevel, marginTop: top }
      }
    },
    {
      tag: 'img',
  ...
  toDOM: (node: Node) => {
    const { align, indent, textIndent, marginTop } = node.attrs
    let style = ''
