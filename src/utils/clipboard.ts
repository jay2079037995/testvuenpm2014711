  ...
  return new Promise((resolve, reject) => {
    if (navigator.clipboard?.readText) {
      navigator.clipboard.readText().then(text => {
        if (!text) reject(i18n.global.t('toast.clipboardCutEmpty'))
        return resolve(text)
      })
    }
    else reject(i18n.global.t('toast.browseNotClipboard'))
  })
}
...
export const pasteCustomClipboardString = (text: string) => {
  ...
  try {
    ...
    const errorMsg = i18n.global.t('toast.parseEncryptedClipboard') + (e as Error)?.message
