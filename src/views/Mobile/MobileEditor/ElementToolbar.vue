  ...
  <div class="element-toolbar">
    ...
    <div class="content">
      <div class="style" v-if="activeTab === 'style'">
        ...
        <div class="row-block">
          <div class="label">{{ $t('canvalTool.textColor') }}</div>
          <div class="colors">
            <div class="color" 
        ...
        <div class="row-block">
          <div class="label">{{ $t('canvalTool.fillColor') }}</div>
          <div class="colors">
            <div class="color" 
              v-for="color in colors" 
              :key="color"
      ...
      <div class="common" v-if="activeTab === 'common'">
        <ButtonGroup class="row">
          <Button style="flex: 1;" @click="copyElement()"><IconCopy class="icon" /> {{ $t('edit.copy') }}</Button>
          <Button style="flex: 1;" @click="deleteElement()"><IconDelete class="icon" /> {{ $t('edit.delete') }}</Button>
        </ButtonGroup>
        
        <Divider style="margin: 20px 0;" />

        <ButtonGroup class="row">
          <Button style="flex: 1;" @click="orderElement(handleElement!, ElementOrderCommands.TOP)"><IconSendToBack class="icon" /> {{ $t('edit.topping') }}</Button>
          <Button style="flex: 1;" @click="orderElement(handleElement!, ElementOrderCommands.BOTTOM)"><IconBringToFrontOne class="icon" /> {{ $t('edit.bottom') }}</Button>
          <Button style="flex: 1;" @click="orderElement(handleElement!, ElementOrderCommands.UP)"><IconBringToFront class="icon" /> {{ $t('edit.moveUp') }}</Button>
          <Button style="flex: 1;" @click="orderElement(handleElement!, ElementOrderCommands.DOWN)"><IconSentToBack class="icon" /> {{ $t('edit.moveDown') }}</Button>
        </ButtonGroup>
        
        <Divider style="margin: 20px 0;" />

        <ButtonGroup class="row">
          <Button style="flex: 1;" @click="alignElementToCanvas(ElementAlignCommands.LEFT)"><IconAlignLeft class="icon" /> {{ $t('panel.leftAligned') }}</Button>
          <Button style="flex: 1;" @click="alignElementToCanvas(ElementAlignCommands.HORIZONTAL)"><IconAlignVertically class="icon" /> {{ $t('panel.horizontally') }}</Button>
          <Button style="flex: 1;" @click="alignElementToCanvas(ElementAlignCommands.RIGHT)"><IconAlignRight class="icon" /> {{ $t('panel.rightAligned') }}</Button>
        </ButtonGroup>
        <ButtonGroup class="row">
          <Button style="flex: 1;" @click="alignElementToCanvas(ElementAlignCommands.TOP)"><IconAlignTop class="icon" /> {{ $t('panel.alignUp') }}</Button>
          <Button style="flex: 1;" @click="alignElementToCanvas(ElementAlignCommands.VERTICAL)"><IconAlignHorizontally class="icon" /> {{ $t('panel.verticalCenter') }}</Button>
          <Button style="flex: 1;" @click="alignElementToCanvas(ElementAlignCommands.BOTTOM)"><IconAlignBottom class="icon" /> {{ $t('panel.bottomJustify') }}</Button>
        </ButtonGroup>
      </div>
    </div>
  </div>
...
const tabs: TabItem[] = [
  { key: 'style', label: i18n.global.t('canvasTool.style') },
  { key: 'common', label: i18n.global.t('canvasTool.layout') },
]
