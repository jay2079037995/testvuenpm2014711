...
import type { ChartData, ChartDataSingleSeries, ChartOptions, ChartType } from '@/types/slides'
...
const renderChart = () => {
  ...
  if (props.type === 'bar') {
    if (props.options.distributeSeries) {
      const data : ChartDataSingleSeries = {
        labels: props.data.labels,
        legends: props.data.legends,
        series: props.data.series.length > 0 ? props.data.series[0] : []
      }
      chart = new BarChart(chartRef.value, data, options)
    }
    else {
...
const updateChart = () => {
  ...
  else if (props.type === 'bar' && props.options.distributeSeries) {
    const singleData : ChartDataSingleSeries = {
      labels: props.data.labels,
      legends: props.data.legends,
      series: props.data.series.length > 0 ? props.data.series[0] : []
    }
...
.chart-content {
  ...
  --theme-color-1: #e6e6e6;
  --theme-color-2: #e6e6e6;
  --theme-color-3: #e6e6e6;
  --theme-color-4: #e6e6e6;
  --theme-color-5: #e6e6e6;
  --theme-color-6: #e6e6e6;
  --theme-color-7: #e6e6e6;
  --theme-color-8: #e6e6e6;
  --theme-color-9: #e6e6e6;
  --theme-color-10: #e6e6e6;
