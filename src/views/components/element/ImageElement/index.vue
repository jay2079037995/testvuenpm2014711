<template>
  ...
  >
    ...
    <Modal
      v-model:visible="imageDataEditorVisible" 
      :width="280"
    >
    <!--  -->
      <ImageGeneratePanel 
        :data="handleImageElement.src"
        @close="imageDataEditorVisible = false"
...
import ImageGeneratePanel from '@/views/Editor/Toolbar/ElementStylePanel/ImageGeneratePanel.vue'
...
const props = defineProps<{
  ...
  selectElement: (e: MouseEvent | TouchEvent, element: PPTImageElement, canMove?: boolean, pageId?: string) => void
...
const imageDataEditorVisible = ref(false)
...
const handleSelectElement = (e: MouseEvent | TouchEvent, canMove = true) => {
  ...
  props.selectElement(e, props.elementInfo, canMove, props.pageId)
}
...
const updateElement = (props: Partial<PPTImageElement>) => {
  // slidesStore.updateElement({ id: handleElementId.value, props })
  // addHistorySnapshot()
}

const updateData = (src: string) => {
  imageDataEditorVisible.value = false
  updateElement({ src })
}

const openDataEditor = () => {
  imageDataEditorVisible.value = true
}
