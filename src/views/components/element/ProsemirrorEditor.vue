...
import { editElementText, needYJS, yjsSetTextChangeBack, yjsGetTextXML, yjsGetAwareness, setEditorElementInput, yjsSetTableTextChangeBack } from '@/utils/yjs-init'
...
const handleInput = debounce(function() {
  // console.log('update text:', editorView.dom.innerHTML, textContent.value)
  if (needYJS) {
    // if (editorView.dom.innerHTML.length < lastText.length) {
    //   console.log('删除文字')
    // } 
    // else {
    //   console.log('添加文字')
    // }
    // lastText = editorView.dom.innerHTML
    emit('update', editorView.dom.innerHTML)
  }
  else {
    emit('update', editorView.dom.innerHTML)
    // if (needYJS) {
    //   editElementText(props.elementId, editorView.dom.innerHTML)
    // }
  }
}, 300, { trailing: true })
...
const onTextChange = (elementId:string, text:string) => {
...
const execCommand = ({ target, action }: RichTextCommand) => {
  if (props.slideId && currentSlide.value?.id !== props.slideId) return
  console.log('execCommand rich 1 text target:%o, props:%o', target, props)
  if (!target && handleElementId.value !== props.elementId) return
  console.log('execCommand rich 2 text handleElementId.value:%o, props.elementId:%o', handleElementId.value, props.elementId)
  if (target && target !== props.elementId) return
  const actions = ('command' in action) ? [action] : action
  console.log('execCommand rich 3 text target:%o, actions:%o', target, actions)
  for (const item of actions) {
    if (item.command === 'fontname' && item.value) {
    ...
    else if (item.command === 'shadow') {
      if (item.value) {
        const { h, v, blur, color } = JSON.parse(item.value)
        const mark = editorView.state.schema.marks.shadow.create({shadow: `${h}px ${v}px ${blur}px ${color}`})
        autoSelectAll(editorView)
        addMark(editorView, mark)
      }
      else {
        toggleMark(editorView.state.schema.marks.shadow)(editorView.state, editorView.dispatch)
      }
    }
      ...
      let mark = editorView.state.schema.marks.fontname.create({ fontname: 'Arial' })
      addMark(editorView, mark)

      mark = editorView.state.schema.marks.fontsize.create({ fontname: 'Arial', fontsize: '14px' })
      addMark(editorView, mark)
      setListStyle(editorView, { key: 'fontsize', value: '14px' })
    }
...
onMounted(() => {
  if (needYJS) {
    const xml = yjsGetTextXML(props.elementId)
...
const inputPosChange = () => {
  setTimeout(() => {
    ...
    containerRect.x = editorViewRef?.value?.parentElement?.parentElement?.parentElement?.getBoundingClientRect().x || 0
    containerRect.y = editorViewRef?.value?.parentElement?.parentElement?.parentElement?.getBoundingClientRect().y || 0
    const rect = range.getBoundingClientRect()
    setEditorElementInput(props.elementId, {
