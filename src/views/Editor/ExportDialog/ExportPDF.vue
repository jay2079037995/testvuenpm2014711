  ...
  <div class="export-pdf-dialog">
    ...
    <div class="configs">
      <div class="row">
        ...
        >
          <RadioButton style="width: 50%;" value="all">{{ $t('all') }}</RadioButton>
          <RadioButton style="width: 50%;" value="current">{{ $t('panel.currentPage') }}</RadioButton>
        </RadioGroup>
      </div>
      <div class="row">
        <div class="title">{{ $t('panel.exportPageNum') }}</div>
        <Select 
          class="config-item"
          v-model:value="count"
          :options="[
      ...
      <div class="row">
        <div class="title">{{ $t('panel.exportEdges') }}</div>
        <div class="config-item">
          <Switch v-model:value="padding" />
        </div>
      </div>
      <div class="tip">
        {{ $t('panel.exportPdfTip') }}
      </div>
    </div>

    <div class="btns">
      <Button class="btn export" type="primary" @click="expPDF()">{{ $t('panel.exportPdf') }}</Button>
