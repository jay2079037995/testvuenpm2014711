<template>
  <div class="export-img-dialog">
    ...
    <div class="configs">
      <div class="row">
        <div class="title">{{ $t('panel.exportFormat') }}</div>
        <RadioGroup
          class="config-item"
      ...
      <div class="row">
        <div class="title">{{ $t('panel.imageQuality') }}</div>
        <Slider
          class="config-item"
          :min="0"
          :max="1"
      ...
      <div class="row">
        <div class="title">{{ $t('panel.ignoringOnlineFonts') }}</div>
        <div class="config-item">
          <Switch v-model:value="ignoreWebfont" v-tooltip="$t('panel.ignoringFontsDesc')" />
        </div>
      </div>
    </div>

    <div class="btns">
      <Button class="btn export" type="primary" @click="expImage()">{{ $t('panel.exportImage') }}</Button>
      <Button class="btn close" @click="emit('close')">{{ $t('close')}}</Button>
    </div>

    <FullscreenSpin :loading="exporting" :tip="$t('exporting')" />
  </div>
</template>
