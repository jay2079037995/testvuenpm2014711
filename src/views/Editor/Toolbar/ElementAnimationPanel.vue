  ...
  <div class="element-animation-panel">
    <div class="element-animation" v-if="handleElement">
      ...
      >
        ...
        <Button class="element-animation-btn" @click="handleAnimationId = ''">
          <IconEffects style="margin-right: 5px;" /> {{ $t('edit.addAnimation') }}
        </Button>
      </Popover>
    </div>

    <div class="tip" v-else><IconClick style="margin-right: 5px;" /> {{ $t('edit.elementAddAnimation') }}</div>
      ...
      <template #item="{ element }">
        <div class="sequence-item" :class="[element.type, { 'active': handleElement?.id === element.elId }]">
          <div class="sequence-content">
            ...
            <div class="handler">
              <IconPlayOne class="handler-btn" v-tooltip="$t('edit.preview')" @click="runAnimation(element.elId, element.effect, element.duration)" />
              <IconCloseSmall class="handler-btn" v-tooltip="$t('edit.delete')" @click="deleteAnimation(element.id)" />
            </div>
          </div>
          ...
          <div class="configs" v-if="handleElementAnimation[0]?.elId === element.elId">
            ...
            <div class="config-item">
              <div style="width: 35%;">{{ $t('edit.duration') }}</div>
              <NumberInput 
                :min="500"
                :max="3000"
                :step="500"
            ...
            <div class="config-item">
              <div style="width: 35%;">{{ $t('edit.trigger') }}</div>
              <Select
                :value="element.trigger"
                @update:value="value => updateElementAnimationTrigger(element.id, value as 'click' | 'meantime' | 'auto')"
                style="width: 65%;"
                :options="[
                  { label: $t('edit.activeTrigger'), value: 'click' },
                  { label: $t('edit.sameAsPrevious'), value: 'meantime' },
                  { label: $t('edit.afterPrevious'), value: 'auto' },
                ]"
              />
            </div>
            <div class="config-item">
              <Button style="width: 100%;" @click="openAnimationPool(element.id)">{{ $t('edit.changeAnimation') }}</Button>
            </div>
          </div>
        </div>
      </template>
    ...
    <template v-if="animationSequence.length >= 2">
      ...
      <Button @click="runAllAnimation()">
        {{ animateIn ? $t('edit.stopPreview') : $t('edit.previewAll')}}
      </Button>
    </template>
  </div>
</template>
...
import { yjsUpdateAnimations } from '@/utils/yjs-init'
...
const tabs: TabItem[] = [
  { key: 'in', label: $t('edit.in'), color: '#68a490' },
  { key: 'out', label: $t('edit.out'), color: '#d86344' },
  { key: 'attention', label: $t('edit.attention'), color: '#e8b76a' },
]
...
const deleteAnimation = (id: string) => {
  ...
  yjsUpdateAnimations(slidesStore.slides[slidesStore.slideIndex].id, animations)
  addHistorySnapshot()
}
