<template>
  <div class="slide-animation-panel">
    ...
    <Button style="width: 100%;" @click="applyAllSlide()">{{ $t('edit.applyAll') }}</Button>
  </div>
</template>
...
import { yjsUpdateTurnningMode } from '@/utils/yjs-init'
...
const updateTurningMode = (mode: TurningMode) => {
  ...
  yjsUpdateTurnningMode(slidesStore.slides[slidesStore.slideIndex].id, mode)
  addHistorySnapshot()
}
...
const applyAllSlide = () => {
  const newSlides = slides.value.map(slide => {
    yjsUpdateTurnningMode(slide.id, currentSlide.value.turningMode)
    return {
      ...slide,
  ...
  message.success($t('edit.appliedToAll'))
  addHistorySnapshot()
}
</script>
