<template>
  <div class="element-color-mask">
    <div class="row">
      <div style="width: 40%;">{{ $t('panel.coloring') }}</div>
    ...
    <template v-if="hasColorMask">
      <div class="row" style="margin-top: 15px;">
        <div style="width: 40%;">{{ $t('panel.maskColor') }}</div>
...
import { yjsSetElementAttributes, yjsDeleteElementAttributes } from '@/utils/yjs-init'
...
const toggleColorMask = (checked: boolean) => {
  ...
  if (checked) {
    ...
    yjsSetElementAttributes(handleElement.value.id, { colorMask: defaultColorMask })
  }
  else {
    slidesStore.removeElementProps({ id: handleElement.value.id, propName: 'colorMask' })
    yjsDeleteElementAttributes(handleElement.value.id, ['colorMask'])
  }
  addHistorySnapshot()
}
...
const updateColorMask = (colorMask: string) => {
  ...
  addHistorySnapshot()
  yjsSetElementAttributes(handleElementId.value, { colorMask })
}
</script>
