<template>
  <div class="element-flip">
    <ButtonGroup class="row">
      ...
      ><IconFlipVertically /> {{ $t('panel.verticalFlip') }}</CheckboxButton>
      <CheckboxButton 
        style="flex: 1;"
        :checked="flipH"
        @click="updateFlip({ flipH: !flipH })"
      ><IconFlipHorizontally /> {{ $t('panel.horizontalFlip') }}</CheckboxButton>
    </ButtonGroup>
  </div>
</template>
...
const updateFlip = (flipProps: ImageOrShapeFlip) => {
  ...
  addHistorySnapshot()
  yjsSetElementAttributes(handleElement.value.id, flipProps)
}
</script>
