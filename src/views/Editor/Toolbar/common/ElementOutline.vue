<template>
  <div class="element-outline">
    <div class="row" v-if="!fixed">
      <div style="width: 40%;">{{ $t('panel.borderEnable') }}</div>
...
import { yjsDeleteElement, yjsSetElementAttributes } from '@/utils/yjs-init'
...
const toggleOutline = (checked: boolean) => {
  ...
  if (checked) {
    ...
    yjsSetElementAttributes(handleElement.value.id, { outline: _outline })
  }
  else {
    slidesStore.removeElementProps({ id: handleElement.value.id, propName: 'outline' })
    yjsDeleteElement(handleElement.value.id, ['outline'])
  }
  addHistorySnapshot()
}
</script>
