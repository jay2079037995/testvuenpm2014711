<template>
  <div class="element-positopn-panel">
    <div class="title">{{ $t('edit.level') }}</div>
    <ButtonGroup class="row">
      <Button style="flex: 1;" @click="orderElement(handleElement!, ElementOrderCommands.TOP)"><IconSendToBack class="btn-icon" /> {{ $t('edit.topping') }}</Button>
      <Button style="flex: 1;" @click="orderElement(handleElement!, ElementOrderCommands.BOTTOM)"><IconBringToFrontOne class="btn-icon" /> {{ $t('edit.bottom') }}</Button>
    </ButtonGroup>
    <ButtonGroup class="row">
      <Button style="flex: 1;" @click="orderElement(handleElement!, ElementOrderCommands.UP)"><IconBringToFront class="btn-icon" /> {{ $t('edit.moveUp') }}</Button>
      <Button style="flex: 1;" @click="orderElement(handleElement!, ElementOrderCommands.DOWN)"><IconSentToBack class="btn-icon" /> {{ $t('edit.moveDown') }}</Button>
    </ButtonGroup>

    <Divider />
    
    <div class="title">{{ $t('edit.alignment') }}</div>
    <ButtonGroup class="row">
      <Button style="flex: 1;" v-tooltip="$t('panel.leftAligned')" @click="alignElementToCanvas(ElementAlignCommands.LEFT)"><IconAlignLeft /></Button>
      <Button style="flex: 1;" v-tooltip="$t('panel.horizontally')" @click="alignElementToCanvas(ElementAlignCommands.HORIZONTAL)"><IconAlignHorizontally /></Button>
      <Button style="flex: 1;" v-tooltip="$t('panel.rightAligned')" @click="alignElementToCanvas(ElementAlignCommands.RIGHT)"><IconAlignRight /></Button>
    </ButtonGroup>
    <ButtonGroup class="row">
      <Button style="flex: 1;" v-tooltip="$t('panel.alignUp')" @click="alignElementToCanvas(ElementAlignCommands.TOP)"><IconAlignTop /></Button>
      <Button style="flex: 1;" v-tooltip="$t('panel.verticalCenter')" @click="alignElementToCanvas(ElementAlignCommands.VERTICAL)"><IconAlignVertically /></Button>
      <Button style="flex: 1;" v-tooltip="$t('panel.bottomJustify')" @click="alignElementToCanvas(ElementAlignCommands.BOTTOM)"><IconAlignBottom /></Button>
    </ButtonGroup>
    ...
    <template v-if="handleElement!.type !== 'line'">
      <div class="row">
        ...
        <template v-if="['image', 'shape', 'audio'].includes(handleElement!.type)">
          <IconLock style="width: 10%;" class="icon-btn" v-tooltip="$t('edit.unlockingAspectRatio')" @click="updateFixedRatio(false)" v-if="fixedRatio" />
          <IconUnlock style="width: 10%;" class="icon-btn" v-tooltip="$t('edit.lockingAspectRatio')" @click="updateFixedRatio(true)" v-else />
        </template>
