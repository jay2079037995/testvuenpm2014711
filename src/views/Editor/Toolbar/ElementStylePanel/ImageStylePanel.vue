<template>
  <div class="image-style-panel">
    ...
    <ButtonGroup class="row" passive>
      <Button first style="width: calc(100% / 6 * 5);" @click="clipImage()"><IconTailoring class="btn-icon" /> {{ $t('panel.cropImage') }}</Button>
      <Popover trigger="click" v-model:value="clipPanelVisible" style="width: calc(100% / 6);">
        <template #content>
          <div class="clip">
            <div class="title">{{ $t('panel.byShape') }}</div>
            ...
            <template v-for="typeItem in ratioClipOptions" :key="typeItem.label">
              <div class="title" v-if="typeItem.label">{{ $t('panel.pressLabel', { label: typeItem.label }) }}</div>
    ...
    <FileInput @change="files => replaceImage(files)">
      <Button class="full-width-btn"><IconTransform class="btn-icon" /> {{ $t('panel.replaceImage') }}</Button>
    </FileInput>
    <Button class="full-width-btn" @click="resetImage()"><IconUndo class="btn-icon" /> {{ $t('panel.resetStyle') }}</Button>
    <Button class="full-width-btn" @click="setBackgroundImage()"><IconTheme class="btn-icon" /> {{ $t('panel.setBackground') }}</Button>
  </div>
</template>
...
import { yjsDeleteElementAttributes, yjsSetElementAttributes, yjsSetSlideAttributes } from '@/utils/yjs-init'

const shapeClipPathOptions = CLIPPATHS
const ratioClipOptions = [
  {
    label: i18n.global.t('panel.aspectRatioSquare'),
    children: [
      { key: '1:1', ratio: 1 / 1 },
    ],
  },
  {
    label: i18n.global.t('panel.aspectRatioVertical'),
    children: [
      { key: '2:3', ratio: 3 / 2 },
      { key: '3:4', ratio: 4 / 3 },
      { key: '3:5', ratio: 5 / 3 },
  ...
  {
    label: i18n.global.t('panel.aspectRatioSquareHorizontal'),
    children: [
      { key: '3:2', ratio: 2 / 3 },
      { key: '4:3', ratio: 3 / 4 },
      { key: '5:3', ratio: 3 / 5 },
...
const presetImageClip = (shape: string, ratio = 0) => {
  ...
  clipImage()
  addHistorySnapshot()
  yjsSetElementAttributes(handleElementId.value, {
    clip: { ..._handleElement.clip, shape, range: originClipRange }
  })
}
...
const resetImage = () => {
  ...
  addHistorySnapshot()
  yjsDeleteElementAttributes(handleElementId.value, ['clip', 'outline', 'flip', 'shadow', 'filters', 'colorMask'])
}
...
const setBackgroundImage = () => {
  ...
  addHistorySnapshot()
  yjsSetSlideAttributes({ background })
}
</script>
