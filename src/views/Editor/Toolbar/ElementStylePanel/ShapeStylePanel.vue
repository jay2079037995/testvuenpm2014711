<template>
  <div class="shape-style-panel">
    <div class="title">
      <span>{{ $t('panel.clickeplaceShape') }}</span>
      <IconDown />
    </div>
    ...
    <template v-if="fillType === 'gradient'">
      <div class="row">
        <div style="width: 40%;">{{ $t('panel.startColor') }}</div>
      ...
      <div class="row">
        <div style="width: 40%;">{{ $t('panel.endColor') }}</div>
        <Popover trigger="click" style="width: 60%;">
          <template #content>
            <ColorPicker
      ...
      <div class="row" v-if="gradient.type === 'linear'">
        <div style="width: 40%;">{{ $t('panel.gradientAngle') }}</div>
        <Slider
          style="width: 60%;"
          :min="0"
          :max="360"
    ...
    <template v-if="handleShapeElement.text?.content">
      ...
      >
        ...
        <RadioButton value="center" v-tooltip="$t('panel.center')" style="flex: 1;"><IconAlignTextCenter /></RadioButton>
      ...
      >
        <RadioButton value="top" v-tooltip="$t('panel.topAlignment')" style="flex: 1;"><IconAlignTextTopOne /></RadioButton>
        <RadioButton value="middle" v-tooltip="$t('panel.center')" style="flex: 1;"><IconAlignTextMiddleOne /></RadioButton>
        <RadioButton value="bottom" v-tooltip="$t('panel.bottomAligned')" style="flex: 1;"><IconAlignTextBottomOne /></RadioButton>
      </RadioGroup>
...
const updateFillType = (type: 'gradient' | 'fill') => {
  if (type === 'fill') {
    ...
    addHistorySnapshot()
    yjsDeleteElementAttributes(handleElementId.value, ['gradient'])
  }
