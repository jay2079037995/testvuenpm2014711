<template>
  <div class="chart-style-panel">
    <Button class="full-width-btn" @click="chartDataEditorVisible = true">
      <IconEdit class="btn-icon" /> {{ $t('edit.editChartData') }}
    </Button>
    ...
    <div class="row" v-if="handleChartElement.chartType === 'bar'">
      <Checkbox 
        ...
        style="flex: 1;"
      >{{ $t('edit.barChartStyle') }}</Checkbox>
      <Checkbox 
        @update:value="value => updateOptions({ stackBars: value })" 
        :value="stackBars"
        style="flex: 1;"
      >{{ $t('edit.stackingStyle') }}</Checkbox>
    </div>
    <div class="row" v-if="handleChartElement.chartType === 'pie'">
      <Checkbox 
        @update:value="value => updateOptions({ donut: value })" 
        :value="donut"
      >{{ $t('edit.circularChartStyle') }}</Checkbox>
    </div>

    <Divider />

    <div class="row">
      <div style="width: 40%;">{{ $t('edit.legend') }}</div>
      <Select 
        style="width: 60%;" 
        :value="legend" 
        @update:value="value => updateLegend(value as '' | 'top' | 'bottom')"
        :options="[
          { label: $t('edit.doNotDisplay'), value: '' },
          { label: $t('edit.displayAbove'), value: 'top' },
          { label: $t('edit.displayBelow'), value: 'bottom' },
        ]"
      />
    </div>
    ...
    <div class="row">
      <div style="width: 40%;">{{ $t('edit.backgroundFill') }}</div>
    ...
    <div class="row">
      <div style="width: 40%;">{{ $t('edit.gridColor') }}</div>
      <Popover trigger="click" style="width: 60%;">
        <template #content>
          <ColorPicker
    ...
    <div class="row" v-for="(color, index) in themeColor" :key="index">
      <div style="width: 40%;">{{index === 0 ? $t('edit.themeColorScheme') : ''}}</div>
      <Popover trigger="click" style="width: 60%;">
        <template #content>
          <ColorPicker
            :modelValue="color"
        ...
        <div class="color-btn-wrap" style="width: 100%;">
          ...
          <div class="delete-color-btn" v-tooltip="$t('edit.delete')" @click.stop="deleteThemeColor(index)" v-if="index !== 0"><IconCloseSmall /></div>
        </div>
      </Popover>
    </div>
    <ButtonGroup class="row" passive>
      <Popover trigger="click" v-model:open="presetThemesVisible" style="width: 40%;">
        ...
        <Button first style="width: 100%;">{{ $t('edit.recommendedThemes') }}</Button>
      </Popover>
      <Button
        last
        ...
        @click="addThemeColor()"
      >
        <IconPlus class="btn-icon" /> {{ $t('edit.addThemeColors') }}
      </Button>
    </ButtonGroup>
