<template>
  <div class="link-dialog">
    ...
    <Input 
      ...
      :placeholder="$t('toast.placeholderWebLink')"
    />
    ...
    <div class="btns">
      ...
      <Button type="primary" @click="save()">{{ $t('sure') }}</Button>
    </div>
  </div>
</template>
...
const slideOptions = computed(() => {
  return slides.value.map((item, index) => ({
    label: i18n.global.t('slideShow.slidePageNum', { index: (index + 1) }),
    value: item.id,
...
const tabs: TabItem[] = [
  { key: 'web', label: i18n.global.t('link.linkWeb') },
  { key: 'slide', label: i18n.global.t('slideShow.slidePage') },
]
