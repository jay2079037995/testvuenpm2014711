...
const resizeHandlers = computed(() => {
  ...
  if (props.elementInfo.brokens && props.elementInfo.brokens.length > 0) {
    const length = props.elementInfo.brokens.length
    if (length === 1) {
      const ctrlHandler = props.elementInfo.brokens[0] as [number, number]
      handlers.push({
      ...
      const ctrlHandler1 = props.elementInfo.brokens[0] as [number, number]
      const ctrlHandler2 = props.elementInfo.brokens[1] as [number, number]
      handlers.push({
      ...
      // TODO brokens.length > 2
    }
  }
  else if (props.elementInfo.curve) {
    const ctrlHandler = props.elementInfo.curve as [number, number]
    handlers.push({
      handler: OperateLineHandlers.C,
      style: {
        left: ctrlHandler[0] * canvasScale.value + 'px',
