<template>
  <div class="link-handler" :style="{ top: height * canvasScale + 10 + 'px' }">
    ...
    <a class="link" v-else @click="turnTarget(link.target)">{{ $t('slideShow.slideTarget', { target: link.target }) }}</a>
    <div class="btns">
      <div class="btn" @click="openLinkDialog()">{{ $t('slideShow.replace') }}</div>
      <Divider type="vertical" />
      <div class="btn" @click="removeLink(elementInfo)">{{ $t('panel.remove') }}</div>
    </div>
  </div>
</template>
