...
export default (elementList: Ref<PPTElement[]>) => {
  ...
  const dragLineElement = (e: MouseEvent, element: PPTLineElement, command: OperateLineHandlers) => {
    ...
    document.onmousemove = e => {
      ...
      let mid: number[] = [0, 0]
      let brokenL: Array<[number, number]> = []
      if (element.brokens && element.brokens.length === 1) {
        mid = element.brokens[0]
      }
      else if (element.brokens && element.brokens.length > 1) {
        brokenL = element.brokens.map(broken => [element.left + broken[0], element.top + broken[1]])
      }
      else if (element.curve) {
        mid = Array.isArray(element.curve) ? element.curve : [0, 0]
      }
        ...
        if (el.id === element.id) {
          ...
          if (command === OperateLineHandlers.START || command === OperateLineHandlers.END) {
            if (ctrlOrShiftKeyActive.value) {
              if (element.brokens && element.brokens.length === 1) {
                newEl.brokens = [[midX - minX, midY - minY]]
              }
              else if (element.brokens && element.brokens.length > 1) {
                newEl.brokens = brokenL.map(broken => [broken[0] - minX, broken[1] - minY])
              }
              ...
              if (element.brokens && element.brokens.length === 1) {
                newEl.brokens = [[(start[0] + end[0]) / 2, (start[1] + end[1]) / 2]]
              }
              else if (element.brokens && element.brokens.length > 1) {
                newEl.brokens = brokenL.map(() => [(start[0] + end[0]) / 2, (start[1] + end[1]) / 2])
              }
