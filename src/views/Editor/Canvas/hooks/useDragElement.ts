...
export default (
  ...
  const dragElement = (e: MouseEvent | TouchEvent, element: PPTElement) => {
    ...
    const handleMouseup = (e: MouseEvent | TouchEvent) => {
      ...
      // console.log('drag over:', elementList.value)
      for (let i = 0; i < elementList.value.length; i++) {
        if (activeElementIdList.value.indexOf(elementList.value[i].id) === -1) continue
        let midY = elementList.value[i].top + (elementList.value[i] as any).height / 2
        if (!(midY < -30 / canvasScale.value || midY > viewportStyles.value.height + 30 / canvasScale.value)) continue
        const pageIndex = mainStore.getElementSlideIndex(elementList.value[i], slideIndex.value)
        const str = JSON.stringify(elementList.value[i])
        const json = JSON.parse(str)
        json.id = nanoid(6)
        let pageOff = 0
        if (midY < -30 / canvasScale.value) {
          while (midY < -30 / canvasScale.value) {
            if (pageIndex + pageOff >= 0) {
              pageOff--
              json.top += viewportStyles.value.height + 30 - 30 * (1 - 1 / canvasScale.value)
              midY = json.top + (elementList.value[i] as any).height / 2
            }
            else {
              break
            }
        ...
        if (midY > viewportStyles.value.height + 30 / canvasScale.value) {
          while (midY > viewportStyles.value.height + 30 / canvasScale.value) {
            if (pageIndex + pageOff < slidesStore.slides.length) {
              pageOff++
              json.top -= viewportStyles.value.height + 30 - 30 * (1 - 1 / canvasScale.value)
              midY = json.top + (elementList.value[i] as any).height / 2
            }
        ...
        if (pageOff !== 0) {
          slidesStore.addElement(json, slidesStore.slides[pageIndex + pageOff].id)
          elementList.value.splice(i, 1)
          i--
          setTimeout(() => {
            slidesStore.updateSlideIndex(pageIndex + pageOff)
            mainStore.setActiveElementIdList([json.id])
          })
        }
      }
