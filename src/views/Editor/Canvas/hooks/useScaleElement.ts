...
export default (
  ...
  const scaleElement = (e: MouseEvent | TouchEvent, element: Exclude<PPTElement, PPTLineElement>, command: OperateResizeHandlers) => {
    ...
    const elOriginContent = (element as any).content
    ...
    const fixedRatio = ctrlOrShiftKeyActive.value || ('fixedRatio' in element && element.fixedRatio) || element.type === 'text'
    ...
    const handleMousemove = (e: MouseEvent | TouchEvent) => {
      ...
      // console.log('缩放元素', elementList.value)
      // if (command === 'left-top' || command === 'right-bottom') {
      //   y = x * height / width
      // }
      // else {
      //   y = -x * height / width
      ...
      elementList.value = elementList.value.map(el => {
        ...
        // console.log(el)
        // <p style="margin-top: 0px;"><span style="font-size: 20px;">123</span></p>
        if (el.type === 'text' && (command === OperateResizeHandlers.LEFT_TOP ||
          ...
          let str:string = elOriginContent
          if (str.indexOf('font-size') === -1) {
            str = '<span style="font-size: 20px;">' + str + '</span>'
          }
          // 寻找所有的 font-size
          // console.log('开始缩放', width, elOriginWidth, ~~(100 * width / elOriginWidth), elOriginContent)
          let start = 0
          while (start < str.length) {
            const index = str.indexOf('font-size', start)
            if (index === -1) break
            const index2 = str.indexOf('px', index)
            const fontTxt = str.slice(index, index2 + 2)
            const fontSize = +(fontTxt.match(/(\d+)/g) + '')
            str = str.slice(0, index) + 'font-size: ' + ~~(fontSize * height / elOriginHeight) + 'px' + str.slice(index2 + 2)
            // console.log(index, index2, str.slice(index, index2 + 2), fontSize, ~~(fontSize * width / elOriginWidth))
            start = index2 + 2
          }
          // console.log('结束缩放', str)
          el.content = str
          // const fontSize:any = el.content.match(/font-size: (\d+)px/)
          // console.log(el.content, fontSize[0])
        }
