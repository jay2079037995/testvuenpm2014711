...
export default (
  ...
  const selectElement = (e: MouseEvent | TouchEvent, element: PPTElement, startMove = true, pageId?: string) => {
    if (!editorAreaFocus.value) mainStore.setEditorareaFocus(true)
    mainStore.updateSelectedSlidesIndex([])
    // const pageIndex = mainStore.getElementSlideIndex(element)
    const pageIndex = slidesStore.getSlideIndexById(pageId)
    if (mainStore.selectedSlidesIndex.length > 1 || mainStore.selectedSlidesIndex[0] !== pageIndex) {
