<template>
  <MoveablePanel 
    ...
    :width="200" 
    :height="360" 
    :title="`${$t('canvasTool.select')}（${activeElementIdList.length}/${currentSlide.elements.length}）`" 
    :left="-270" 
    :top="90"
    @close="close()"
  >
    <div class="handler" v-if="elements.length">
      <div class="btns">
        <Button size="small" style="margin-right: 5px;" @click="showAll()">{{ $t('canvasTool.allShow') }}</Button>
        <Button size="small" @click="hideAll()">{{ $t('canvasTool.allHide') }}</Button>
      </div>
    ...
    <div class="element-list">
      <template v-for="item in elements" :key="item.id">
        <div class="group-els" v-if="item.type === 'group'">
          <div class="group-title">{{ $t('canvasTool.group') }}</div>
          <div 
            class="item" 
            :class="{
