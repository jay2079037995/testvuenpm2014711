...
export default () => {
  ...
  const execPrev = () => {
    if (formatedAnimations.value.length && animationIndex.value > 0) {
      ...
      else throttleMassage(i18n.global.t('slideShow.alreadyFirstPage'))
    }
    inAnimation.value = false
  }
  const execNext = () => {
    if (formatedAnimations.value.length && animationIndex.value < formatedAnimations.value.length) {
      ...
      if (loopPlay.value) turnSlideToIndex(0)
        ...
        throttleMassage(i18n.global.t('slideShow.alreadyLastPage'))
        closeAutoPlay()
      }
      inAnimation.value = false
    }
  ...
  const autoPlay = () => {
    closeAutoPlay()
    message.success(i18n.global.t('slideShow.startAutoPlay'))
