<template>
  <div class="presenter-view">
    <div class="toolbar">
      <div class="tool-btn" @click="changeViewMode('base')"><IconListView class="tool-icon" /><span>{{ $t('panel.normal') }}</span></div>
      <div class="tool-btn" :class="{ 'active': writingBoardToolVisible }" @click="writingBoardToolVisible = !writingBoardToolVisible"><IconWrite class="tool-icon" /><span>{{ $t('panel.brush') }}</span></div>
      <div class="tool-btn" :class="{ 'active': laserPen }" @click="laserPen = !laserPen"><IconMagic class="tool-icon" /><span>{{ $t('panel.laserPen') }}</span></div>
      <div class="tool-btn" :class="{ 'active': timerlVisible }" @click="timerlVisible = !timerlVisible"><IconStopwatchStart class="tool-icon" /><span>{{ $t('panel.timer') }}</span></div>
      <div class="tool-btn" @click="() => fullscreenState ? manualExitFullscreen() : enterFullscreen()">
        ...
        <span>{{ fullscreenState ? $t('slideShow.exitFullScreen') : $t('slideShow.fullScreen') }}</span>
      </div>
      <Divider class="divider" />
      <div class="tool-btn" @click="exitScreening()"><IconPower class="tool-icon" /><span>{{ $t('slideShow.endScreening') }}</span></div>
    </div>
...
const currentSlideRemark = computed(() => {
  return parseText2Paragraphs(currentSlide.value.remark || i18n.global.t('edit.noRemark'))
})
...
const contextmenus = (): ContextmenuItem[] => {
  return [
    ...
    {
      text: i18n.global.t('panel.normal'),
      handler: () => props.changeViewMode('base'),
    },
    { divider: true },
    {
      text: i18n.global.t('slideShow.endScreening'),
      subText: 'ESC',
