      ...
      <div class="tools" @mousedown.stop>
        <div class="tool-content">
          <Popover trigger="manual" :value="sizePopoverType === 'pen'">
            <template #content>
              <div class="size">
                <div class="label">{{ $t('panel.inkThickness') }}</div>
                <Slider class="size-slider" :min="4" :max="10" :step="2" v-model:value="penSize" />
              </div>
            </template>
            <div class="btn" :class="{ 'active': writingBoardModel === 'pen' }" v-tooltip="$t('panel.brush')" @click="changeModel('pen')">
          ...
          <Popover trigger="manual" :value="sizePopoverType === 'mark'">
            ...
            <div class="btn" :class="{ 'active': writingBoardModel === 'mark' }" v-tooltip="$t('panel.highlighterPen')" @click="changeModel('mark')">
          ...
          <Popover trigger="manual" :value="sizePopoverType === 'eraser'">
            <template #content>
              <div class="size">
                <div class="label">{{ $t('panel.rubberSize') }}</div>
                <Slider class="size-slider" :min="20" :max="200" :step="20" v-model:value="rubberSize" />
              </div>
            </template>
            <div class="btn" :class="{ 'active': writingBoardModel === 'eraser' }" v-tooltip="$t('panel.eraser')" @click="changeModel('eraser')">
              <IconErase class="icon" />
            </div>
          </Popover>
          <div class="btn" v-tooltip="$t('panel.clearInk')" @click="clearCanvas()">
            <IconClear class="icon" />
          </div>
          <div class="btn" :class="{ 'active': blackboard }" v-tooltip="$t('panel.blackboard')" @click="blackboard = !blackboard">
            <IconFill class="icon" />
          </div>
          <div class="colors">
            <div 
        ...
        <div class="btn" v-tooltip="$t('panel.turnOffBrushes')" @click="closeWritingBoard()">
